<section class="max-w-4xl mx-auto p-12 shadow-xl rounded">
  ...
  <form method="POST" action="/modify-article/<?php echo $post->getId(); ?>" enctype="multipart/form-data">
    <div class="mb-2">
    ...
    <div class="mb-2">
      ...
      <textarea class="border border-solid border-gray-300 h-48 appearance-none rounded w-full px-3 py-3 focus focus:border-amber-600 focus:outline-none active:outline-none active:border-v-600" id="content" name="content"><?php echo $post->getContent();?>
      </textarea>
    </div>              
    <div class="mb-2">
      <label for="formFile" class="font-sans font-medium text-gray-700">Add an Image</label>
      <input class="appearance-none block w-full px-3 py-1.5 text-base font-normal m-0 text-gray-700 bg-white bg-clip-padding border border-solid border-gray-300 rounded focus:text-gray-700 focus:bg-white focus:border-amber-800 focus:outline-none" type="file" id="formFile" name="image" value="<?php echo $post->getPostThumbnail(); ?>">
    </div>
